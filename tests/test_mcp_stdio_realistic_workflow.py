...
async def scenario() -> None:
    async with open_client_session() as session:
        ...
        assert payload["interpreter_output"].splitlines() == ["16.2", "21"]
        ...
        payload = tool_json(result)
        slope, intercept = payload["interpreter_output"].splitlines()
        ...
        payload = tool_json(result)
        assert "cost + 3" in payload["interpreter_output"]
        assert "15" in payload["interpreter_output"]
        ...
        payload = tool_json(result)
        assert payload["interpreter_output"] == "750\n"
